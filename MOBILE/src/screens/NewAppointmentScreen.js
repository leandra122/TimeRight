...
export default function NewAppointmentScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const selectionChanged =
      currentSelection.current.funcionarioId !== employee.id
  ...
  function changeDate(nextDate, nativeDate = null) {
    ...
    // O calendário pode informar novamente a mesma data ao ser fechado.
    // Nesse caso, preservamos os horários e a seleção já carregados.
  ...
  function changeNativeDate(event, value) {
    ...
    // Fechar o calendário sem escolher uma nova data
    // não deve apagar os horários disponíveis.
  ...
  async function submit() {
    ...
    const submittedContext = availabilityContext(
      selectedDate,
      employee.id,
      service.id
    );
    ...
    if (!isSameContext(currentContext.current, submittedContext)) {
      return;
    }
    ...
    try {
      ...
      navigation.replace('AppointmentConfirmation', {
        appointment: data,
      });
    } catch (submitError) {
      const canApply =
  ...
  return (
    <Screen>
      <Title
        eyebrow="Etapas 3 e 4 de 5"
        subtitle="A disponibilidade é consultada em tempo real."
      >
        Data e horário
      </Title>
      ...
      {!availabilityLoading && availabilityError ? (
        <View>
          ...
          <Button
            ...
            disabled={submitting}
            onPress={() =>
              loadAvailability(currentContext.current)
            }
          />
        </View>
      ) : null}
